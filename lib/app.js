...
function drawEarth(){
	...
	console.log(inverseTransposeMatrix);
...
function printMousePos(event) {
	w = event.clientX - canvas.width/2;
	h =  event.clientY -canvas.height/2;
	alert(
		"clientX: " + w +
		" - clientY: " + h)
	}
...
function makeTextCanvas(text, width, height) {
	...
	// canvasEl.addEventListener("mouseover", printName(canvasEl.id), false);
...
function drawPin(lat, long, name, index) {
	...
	for (let iterPalazzi = 0; iterPalazzi < numPalazzi; iterPalazzi++) {
		...
		carMatrix = utils.multiplyMatrices(
			utils.multiplyMatrices(
				utils.multiplyMatrices(
					utils.multiplyMatrices(t, s),
					ry)
				,
				rz
			),
		...
		gl.uniformMatrix4fv(program.pMatrixUniform, gl.FALSE, utils.transposeMatrix(WVPmatrix));
		gl.uniformMatrix4fv(program.wMatrixUniform, gl.FALSE, utils.transposeMatrix(carMatrix));

		// let matrixTemp =


		let matrixTemp = carMatrix;
