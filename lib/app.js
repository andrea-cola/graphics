...
function setShader() {
	...
	select_flag = gl.getUniformLocation(program, "select_flag");
...
// 	mouxeY = -(event.clientY - canvas.height / 2);
...
var city_chosen = "NoCity"
var city_overlap = "NoCity"
...
function setMousePos(event) {
	...
	for (const [key, value] of Object.entries(positionDictionary)) {
		...
		if(flagX && flagY && isCityVisible(key)){
...
function isCityVisible(city){
	...
	c.forEach(e => {
		if(e.name ==  city){
			let radiusViewSphere = 3.2;
			if(lookRadius<=6.4)
				radiusViewSphere = lookRadius/2;
			...
			var cityX = fromSphToX(e.lat, e.lng, 3.15);
			var cityY = fromSphToY(e.lat, e.lng, 3.15);
			var cityZ = fromSphToZ(e.lat, e.lng, 3.15);
			// alert(cityX + " " + cityY + " " + cityZ);
			if(distance([x,y,z],[cityX, cityY, cityZ])<=radiusViewSphere){
				flag = true;
			}
			// var vPos = elevation - 90 <= e.lat && e.lat <= elevation + 90;
			// var hPos = angle - 90 <= e.lng && e.lng <= angle + 90;
			// flag = vPos && hPos;
			// console.log
			// alert(flag + " " + distance([x,y,z],[cityX, cityY, cityZ]) + " "+ radiusViewSphere);
		}
	});
	return flag;
}
...
function drawPin(lat, long, name, index) {
	...
	for (let iterPalazzi = 0; iterPalazzi < numPalazzi; iterPalazzi++) {
		...
		if(name==city_chosen || city_overlap == name)
			gl.uniform1f(select_flag, 1);
		else{
			gl.uniform1f(select_flag, 0);
		}
