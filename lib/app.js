...
function printMousePos(event) {
	let w = 2*(event.clientX - canvas.width/2)/canvas.width;
	let h = -2*(event.clientY - canvas.height/2)/canvas.height;
	let pw = utils.multiplyMatrices(viewMatrix, perspectiveMatrix);
	pw = utils.invertMatrix(pw);

	z = 0;
	let point = [w,h,z,1];
	let mat = utils.multiplyMatrixVector(pw, point);
	// console.log(mat);
	let norm = mat[3];
	mat = [mat[0]/norm, mat[1]/norm, mat[2]/norm];
	console.log(
		"clientX: " + w +
		" - clientY: " + h, mat);
	}
