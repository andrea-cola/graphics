...
function drawPin(lat, long){
	var la = utils.degToRad(parseFloat(lat));
	var lo = utils.degToRad(parseFloat(long));
	var x = parseFloat(fromGeoToX(la, lo));
	var y = parseFloat(fromGeoToY(la, lo));
	var z = parseFloat(fromGeoToZ(la, lo));

	// draws the moon
	var t = utils.MakeTranslateMatrix(0.0, 0.0, 3.2);
...
function fromGeoToX(lat, lng){
	var x = 3.2 * Math.cos(lng) * Math.sin(lat);
	return x;
}

function fromGeoToY(lat, lng){
	var y = 3.2 * Math.sin(lng) * Math.sin(lat);
	return y;
}

function fromGeoToZ(lat, lng){
	var z = 3.2 * Math.cos(lat);
	return z;
}