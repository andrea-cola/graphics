...
function drawPin(lat, long){
	var x = parseFloat(fromGeoToX(lat, long));
	var y = parseFloat(fromGeoToY(lat, long));
	var z = parseFloat(fromGeoToZ(lat, long));
...
function fromGeoToX(lat, lng){
	if(lat < 0)
		return -3.2 * Math.cos(utils.degToRad(lng)) * Math.sin(utils.degToRad(lat));
	return 3.2 * Math.cos(utils.degToRad(lng)) * Math.sin(utils.degToRad(lat));
}

function fromGeoToY(lat, lng){
	if(lat < 0)
		return -3.2 * Math.sin(utils.degToRad(lng)) * Math.sin(utils.degToRad(lat));	
	return 3.2 * Math.sin(utils.degToRad(lng)) * Math.sin(utils.degToRad(lat));
}

function fromGeoToZ(lat, lng){
	if(lat < 0)
		return -3.2 * Math.cos(utils.degToRad(lat));
	return 3.2 * Math.cos(utils.degToRad(lat));
}