...
function setWorldMesh() {
	...
	worldScale = 1;
	pinMesh = new OBJ.Mesh(carObjStr);
	imgtx2 = new Image();
	imgtx2.txNum = 1;
...
function setShader() {
	...
	for (var i = 0; i < unifParArray.length; i++) {
		...
		console.log(unifParArray[i].pGLSL + "Uniform");
	}
}
...
function drawScene() {
	...
	console.log(worldMesh.normalBuffer);
...
function drawPin(lat, long, name, index) {
	...
	for (let iterPalazzi = 0; iterPalazzi < numPalazzi; iterPalazzi++) {
		...
		var s = utils.MakeScaleMatrix(0.0002 * rescaleFactor);
		...
		console.log(pinMesh.normalBuffer);
